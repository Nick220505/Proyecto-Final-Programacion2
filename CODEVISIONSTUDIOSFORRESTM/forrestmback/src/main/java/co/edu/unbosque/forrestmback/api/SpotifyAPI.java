...
public class SpotifyAPI {
	...
	public String getTrackId(@PathVariable String name) throws Exception {
		refreshAccessToken();
		SearchItemRequest searchItemRequest = spotifyApi.searchItem(name, "track").build();
		SearchResult searchResult = searchItemRequest.execute();
		if (searchResult.getTracks().getTotal() > 0) {
			String trackId = searchResult.getTracks().getItems()[0].getId();
			return trackId;
		} else {
